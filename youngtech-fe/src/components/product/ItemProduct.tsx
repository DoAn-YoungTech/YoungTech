...
export const ItemProduct = ({ item }) => {
  ...
  const router = useRouter();
  const dispatch = useDispatch<AppDispatch>();
  const { parentName, childName } = useSelector(
    (state: RootState) => state.categories_child.nameCategory
  );
  const handlClickDetailsPro = async (
    id: number,
    name: string,
    ...
    if (parentName && childName && name) {
      ...
      const childCategoryName = slugify(childName);
      router.push(
        `/${parentCategoryName}/${childCategoryName}/${nameProduct}/?id=${id}`
      );
    }
  };
  const formattedPrice = new Intl.NumberFormat("de-DE").format(
    item.productRetailPrice
  );

  const priceRetailSale = new Intl.NumberFormat("de-DE").format(
    item.productRetailPrice -
      item.productRetailPrice * (item.productSalePrice / 100)
  );
  return (
    <div
      onClick={() =>
        handlClickDetailsPro(item.id, item.productName, item.childCategory_id)
      }
      key={item.id}
      className="flex  group flex-col my-3 hover:shadow-lg border px-3 rounded-lg py-3"
    >
      <p className="mb-5 overflow-hidden   text-[11px] w-[70px] flex justify-center bg-slate-200">
        Trả góp 0%
      </p>
      <div className="image relative h-[200px]">
        <Image
      ...
      <div className="print-screen pt-1 w-full flex items-center gap-2">
        <span className="text-[12px] bg-slate-200 px-2 rounded-lg">6.9</span>
        <span className="text-[12px] bg-slate-200 px-2 rounded-lg">
          Super Retina XDR
        </span>
      </div>
      <MemoryGb />
      <strong className="price w-full text-[16px] text-red-600">
        {Number(item.productSalePrice) === 0 ? formattedPrice : priceRetailSale}
        ₫
      </strong>
