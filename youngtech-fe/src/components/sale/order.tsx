...
export default function OrderPage() {
  ...
  const handleSaveOrder = () => {
    ...
    if (
      ...
      !/^[A-Za-z\s]+$/.test(selectedOrder.status)
    ) {
      toast.error(
        "Trạng thái không được để trống và chỉ được nhập chữ. Vui lòng kiểm tra lại."
      );
      return;
    }

    if (
      selectedOrder.orderDate > selectedOrder.succesDate ||
      selectedOrder.orderDate == selectedOrder.succesDate
    ) {
      toast.error(
        "Ngày hoàn thành đơn hàng không được sau ngày đặt hàng. Vui lòng kiểm tra lại."
      );
      return;
    }
    ...
    if (selectedOrder.totalAmount % 1000 !== 0) {
      toast.error(
        "Tổng số tiền phải là bội số của 1.000đ. Vui lòng kiểm tra lại."
      );
      return;
    }
  ...
  const filteredOrders = data.filter((order) => {
    ...
    const dateToCompare =
      searchType === "orderDate" ? order.orderDate : order.succesDate;
    ...
    <div className="order-page p-4">
      ...
      {loading ? (
        ...
        >
          ...
          <Column
            ...
            body={(rowData) => new Date(rowData.orderDate).toLocaleDateString()}
          />
          <Column
            field="succesDate"
            header="Ngày Hoàn Thành"
            body={(rowData) =>
              new Date(rowData.succesDate).toLocaleDateString()
            }
          />
          <Column
            field="totalAmount"
            header="Tổng Số Tiền"
            body={(rowData) =>
              rowData.totalAmount.toLocaleString("vi-VN", {
                style: "currency",
                currency: "VND",
              })
            }
            ...
            body={(rowData) => (
              <>
                ...
                <Button
                  icon="pi pi-trash"
                  className="p-button-text"
                  onClick={() => openConfirmDelete(rowData)}
                />
              </>
            )}
          />
      ...
      >
        <div className="p-4">
            ...
            <InputText
              ...
              value={
                selectedOrder.orderDate instanceof Date
                  ? selectedOrder.orderDate.toISOString().split("T")[0]
                  : ""
              }
              onChange={(e) =>
                setSelectedOrder({
          ...
          <div className="field mb-3">
            <label htmlFor="succesDate" className="block mb-1">
              Ngày hoàn thành
            </label>
            <InputText
              id="succesDate"
              type="date"
              value={
                selectedOrder.succesDate instanceof Date
                  ? selectedOrder.succesDate.toISOString().split("T")[0]
                  : ""
              }
              onChange={(e) =>
                setSelectedOrder({
                  ...selectedOrder,
                  succesDate: new Date(e.target.value),
                })
              }
              className="w-full border border-gray-300 rounded-md p-2"
            />
