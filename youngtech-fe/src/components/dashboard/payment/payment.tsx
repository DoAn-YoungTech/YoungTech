...
export const MainMenusGradientCard = ({
  ...
  return (
    ...
    >
      {withArrow && (
        <ArrowUpRightIcon className="absolute top-2 right-2 z-10 size-5 translate-y-4 text-white opacity-0 transition-all group-hover:translate-y-0 group-hover:opacity-100 dark:text-neutral-300 " />
      )}
      <div
        className={cn(
      ...
      />
      <div className="absolute inset-px rounded-[19px] cursor-pointer  bg-neutral-900/80" />
      {children && (
        <div
          className={cn(
            "gird relative h-40 place-content-center overflow-hidden rounded-[15px] text-white/70   border-neutral-950  bg-black/50",
            className,
          )}
        >
          {children}
