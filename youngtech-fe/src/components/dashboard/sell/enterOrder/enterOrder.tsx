...
const EnterOrder = () => {
  ...
  const fetchProducts = async (page) => {
    try {
      const response = await fetch(`${baseURL}/product?page=${page}&limit=4`); // Gửi request với page và limit
      const result = await response.json();

      setSelectedProducts(result.data); // Dữ liệu sản phẩm
      setCurrentPage(result.currentPage); // Trang hiện tại
      setTotalPages(result.totalPages); // Tổng số trang
    } catch (error) {
    ...
    <>
      ...
      <AlertDialog>
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>
                Bạn đồng ý xác nhận đơn hàng .
            </AlertDialogTitle>
            <AlertDialogDescription>
              Xác nhận đơn hàng , bạn hãy nhấn tiếp tục để tiến trình được
              diễn ra tiếp tục nhé .
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
