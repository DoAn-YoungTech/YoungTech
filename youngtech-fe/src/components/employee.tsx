...
export default function EmployeePage() {
  ...
  const validateEmployee = (employee: Employee, isEdit = false) => {
    ...
    if (
      ...
      employee.phoneNumber !==
        data.find((emp) => emp.id === employee.id)?.phoneNumber
    ) {
    ...
    if (!validPositions.includes(employee.position)) {
      toast.error(
        "Vai trò không hợp lệ! Vui lòng chọn một vai trò từ danh sách."
      );
      return false;
    }
  ...
  const handleSaveEmployee = async (isEdit: boolean) => {
    ...
    const profilePicture = imageFile
      ? await getEmployeeProfilePicture()
  ...
  const handleDeleteEmployee = async (employee: Employee) => {
    const confirmed = window.confirm(
      `Bạn chắc chắn muốn xóa nhân viên ${employee.fullName} chứ?`
    );
    if (!confirmed) return;
  ...
  const handleRestoreEmployee = async (employee: Employee) => {
    const confirmed = window.confirm(
      `Bạn chắc chắn muốn khôi phục nhân viên ${employee.fullName} chứ?`
    );
    if (!confirmed) return;
    try {
  ...
  const filteredEmployees = data.filter(
    (employee: { fullName: string; phoneNumber: string | string[] }) =>
      ...
      >
        <div className="p-fluid space-y-4">
              ...
              onChange={(e) => {
                ...
                if (
                  ...
                  const dateInGMT7 = new Date(
                    selectedDate.getTime() + 7 * 60 * 60 * 1000
                  );
                  setSelectedEmployee({
                    ...selectedEmployee,
          ...
          <Button
            label="Lưu"
            icon="pi pi-check"
            className="mt-2 p-button-success rounded-md"
          ...
          <div className="p-field">
            ...
            <Calendar
              ...
              onChange={(e) =>
                setSelectedEmployee({
                  ...selectedEmployee,
                  dateOfBirth: (e.value as Date).toISOString().split("T")[0],
                })
              }
              dateFormat="dd/mm/yy"
